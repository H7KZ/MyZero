...
```
crates/
  devices/            OLED, LED, button, PIR motion drivers
...
[devices](crates/devices/README.md) · [audio](crates/audio/README.md) · [net](crates/net/README.md) ·
...
`departure-board` cross-compiles cleanly. `voice-assistant` links native libraries (`libvosk`, ALSA) — build it on the
Pi or use `Cross.toml`; see its README. The Rust HW crates are Linux-only, so nothing here compiles on a Windows/macOS
host except `voice-assistant`'s pure-std WoL/unit tests.
